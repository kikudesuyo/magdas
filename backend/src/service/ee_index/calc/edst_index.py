...
class Edst:
    @staticmethod
    def calc_edst(start_ut: datetime, end_dt: datetime) -> np.ndarray:
        days, hours, minutes = DateUtils.time_diff(start_ut, end_dt)
        data_length = (
        ...
        for station in EeIndexStation:
            night_er = np.array([])
            night_er_instance = NightEr(station, start_ut, end_dt)
    ...
    @staticmethod
    def compute_smoothed_edst(start_ut: datetime, end_dt: datetime) -> np.ndarray:
