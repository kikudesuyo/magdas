...
def calc_euel_for_eej_detection(station: EeIndexStation, local_date: date):
    ...
    euel_for_baseline = np.concatenate(
        (
            euel_values[0 : TimeUnit.FIVE_HOURS.min],
            np.nan * np.ones(TimeUnit.FOURTEEN_HOURS.min),
            euel_values[TimeUnit.NINETEEN_HOURS.min : TimeUnit.ONE_DAY.min],
        )
    )
...
def has_night_data(local_daily_data: np.ndarray) -> bool:
    ...
    dawn_e = local_daily_data[0 : TimeUnit.FIVE_HOURS.min]
    dusk_e = local_daily_data[TimeUnit.NINETEEN_HOURS.min : TimeUnit.ONE_DAY.min]
